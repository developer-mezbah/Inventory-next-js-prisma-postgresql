...
const customRenderers = {
  ...
  type: (value) => {
    const typeColors = {
      ...
      "LOAN_PAYMENT": "bg-green-100 text-green-800",
      "LOAN_PROCESSING_FEE": "bg-red-100 text-red-800",
      "LOAN_CHARGE": "bg-orange-100 text-orange-800",
      "LOAN_INCREASE": "bg-purple-100 text-purple-800",
      "REVERSAL": "bg-gray-100 text-gray-800",
    };
    
    // Format the type for display
    const displayType = value ? value.replace(/_/g, ' ') : value;
  ...
  amount: (value, transaction) => {
    ...
    // Negative amounts for certain transaction types
    if (transaction.type === "LOAN_CHARGE" || 
        transaction.type === "LOAN_PROCESSING_FEE" ||
  ...
  status: (value) => {
    ...
    if (value === "REVERSED") statusClass = "bg-red-100 text-red-800";
    if (value === "CANCELLED") statusClass = "bg-gray-100 text-gray-800";
...
const LoanAccounts = ({accordion}) => {
  ...
  // Process the nested data structure to extract all transactions
  const allTransactions = useMemo(() => {
    if (!data || Object.keys(data).length === 0) return [];
    
    const transactions = [];
    
    // Extract transactions from accountData (loan accounts)
    if (data.accountData && Array.isArray(data.accountData)) {
      data.accountData.forEach(account => {
    ...
    // You can also add bank and cash transactions if they exist in your data structure
    // For now, we're focusing on accountData transactions
  ...
  const formatTransactionData = useMemo(() => {
    return allTransactions.map(transaction => {
      ...
      // Determine party/account name
      let partyName = transaction.name || transaction.accountName || "N/A";
      if (!partyName || partyName === "N/A") {
        if (transaction.description) {
          partyName = transaction.description.substring(0, 30);
          if (transaction.description.length > 30) partyName += '...';
        }
      }
      ...
      return {
        id: transaction.id,
        date: date,
        invoiceNo: transaction.invoiceNo || transaction.transactionId || `TRX-${transaction.id?.slice(-6) || '000000'}`,
        partyName: partyName,
        partyId: transaction.partyId,
        type: transaction.type,
        paymentType: transaction.paymentType || "N/A",
        amount: Math.abs(transaction.amount) || 0,
        originalAmount: transaction.amount, // Keep original for sign determination
  ...
  const filteredTransactions = useMemo(() => {
    if (isInitialLoading || error || !formatTransactionData.length) return [];
    ...
    return formatTransactionData.filter((transaction) => {
      // 1. Date Filter
      const transactionDate = transaction.date ? new Date(transaction.date).getTime() : null;
      const isDateMatch = transactionDate && transactionDate >= start && transactionDate <= end;
