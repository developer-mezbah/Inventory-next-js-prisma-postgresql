...
const mapDataForTable = (data, subCategory) => {
    // The prop data is structured as [[...items]], so we check for that and flatten if necessary.
    const items = Array.isArray(data) && Array.isArray(data[0]) ? data[0] : data || [];

    // 1. FILTER the items based on subCategory match
    const filteredItems = items.filter(item => {
        // If no subCategory is provided, show all items (no filter applied)
        if (!subCategory || !subCategory.id) {
            return true;
        }

        // Match: item.subCategoryId must equal subCategory.id
        // Ensure both are treated as numbers or strings for comparison if types are inconsistent
        return item.subCategoryId === subCategory.id;
    });

    // 2. MAP the filtered items to the component's expected structure
    return filteredItems.map(item => {
        // Calculate Stock Value: openingQuantity * salePrice
        const stockValue = (item.stock?.openingQuantity || 0) * (item.purchasePrice || 0);

        return {
            id: item.id,
            name: item.itemName || "Unknown Item",
            quantity: item.stock?.openingQuantity || 0,
...
// Update component props to include subCategory
export default function TransactionsTable({ data, subcategory }) {
    // Pass subCategory to the mapping function
    const initialMappedData = mapDataForTable(data, subcategory);

    const [searchTerm, setSearchTerm] = useState("")
    const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" })
    const [filteredStock, setFilteredStock] = useState(initialMappedData)

    // Reset filteredStock whenever the prop data OR subCategory changes
    useEffect(() => {
        const newMappedData = mapDataForTable(data, subcategory);
        setFilteredStock(newMappedData);
        // Reset search and sort when the base data set changes
        setSearchTerm("");
        setSortConfig({ key: null, direction: "asc" });
    }, [data, subcategory]) // Dependency array includes subCategory
    ...
    const handleSearch = (e) => {
        ...
        // 1. Get the base data already filtered by subCategory
        const allDataFilteredBySubcategory = mapDataForTable(data, subcategory);

        // 2. Apply search filter on the subCategory-filtered data
        const filtered = allDataFilteredBySubcategory.filter(
            (item) =>
        ...
        if (sortConfig.key) {
            // Re-sort the newly filtered data if a sort is currently applied
    ...
    const handleSort = (key) => {
        ...
        // Use the currently filtered list for sorting
        const sorted = sortData(filteredStock, key, direction);
        ...
        <div className="w-full p-4">
            ...
            <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <h1 className="text-3xl font-bold text-gray-900">
                    Inventory Stock
                    {/* {subcategory && subcategory.name && <span className="text-lg text-blue-600"> ({subcategory.name})</span>} */}
                </h1>
                <div className="md:w-1/3 items-center gap-2">
                    ...
                    <div className="">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder={`Search stock in ${subcategory?.name || 'all categories'}...`}
                                value={searchTerm}
                            ...
                            {searchTerm && <IoClose onClick={() => {
                                // Reset filtered stock to the original mapped data (which already includes subCategory filter)
                                setFilteredStock(mapDataForTable(data, subcategory))
            ...
            {filteredStock.length === 0 && (
                <div className="rounded-lg border border-gray-200 bg-gray-50 py-12 text-center">
                    <p className="text-gray-500">No stock items found
                        {subcategory && subcategory.name && ` in the '${subcategory.name}' subcategory`}.
                    </p>
                </div>
            )}
        </div>
