...
// --- Provided Sample Data (Integrated for Filtering Demo) ---
const mockFetchedData = [
  {
    data: [
      {
        id: "69272752a3d0447ce74a39af",
        billDate: "2025-11-26",
        amount: 2000,
        isPaid: false,
        partyName: "Evaly",
        ...
        transaction: [
          {
            amount: 2000,
            type: "Purchase",
            transactionId: "1E6843F0CF",
          },
        ],
      },
      {
        id: "69272400a3d0447ce74a39ab",
        billDate: "2025-11-26",
        amount: 2400,
        isPaid: false,
        partyName: "Daraz",
        ...
        transaction: [
          {
            amount: 2400,
            type: "Purchase",
            transactionId: "800D364952",
          },
        ],
      },
      {
        id: "692723c7a3d0447ce74a39a9",
        billDate: "2025-11-26",
        amount: 2000,
        isPaid: true, // **Changed to true for demo calculation**
        partyName: "TechNova Solutions ",
        paymentType: "Cash",
        transaction: [
          {
            amount: 2000,
            type: "Purchase",
            transactionId: "273033E733",
          },
        ],
      },
      {
        id: "69271cd82edd507f3d17ab2e",
        billDate: "2025-11-26",
        amount: 0,
        isPaid: false,
        partyName: "masud",
...
// Extract and flatten the purchase bills data
// const allPurchaseBills = mockFetchedData[0]?.data || [];
...
const PurchaseTransactionReportPage = ({accordion}) => {
  ...
  const {
    ...
    refetch,
  } = useFetchData("/api/purchase", ["purchase"]);
  const allPurchaseBills = data?.data || [];
  ...
  const filteredBills = useMemo(() => {
    ...
    return allPurchaseBills.filter((bill) => {
      // 1. Date Filter
  ...
  }, [allPurchaseBills, fromDate, toDate, firm, user, isInitialLoading, error]);

  // --- Transaction Summary Calculation ---
  const { paidAmount, unpaidAmount, totalAmount } = useMemo(() => {
    let paid = 0;
    let balance = 0; // Renamed 'unpaid' to 'balance' for clarity based on your input
    let total = 0;

    filteredBills.forEach((bill) => {
      // Ensure properties exist before summing, defaulting to 0
      const billPaidAmount = bill.paidAmount || 0;
      const billBalanceDue = bill.balanceDue || 0;
      const billTotalAmount = bill.amount || 0;

      paid += billPaidAmount;
      balance += billBalanceDue;
      total += billTotalAmount; // This will be the sum of the 'amount' property across all bills
    });

    // Check for consistency: total should ideally equal paid + balance
    return {
      paidAmount: paid,
  ...
  const uniqueFirms = useMemo(() => {
    const firms = new Set(
      allPurchaseBills.map((bill) => bill.partyName).filter(Boolean)
    );
