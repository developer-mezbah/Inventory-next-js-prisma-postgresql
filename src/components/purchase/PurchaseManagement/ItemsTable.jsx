...
import { FiTrash2, FiPlus, FiChevronDown, FiSearch } from "react-icons/fi"
...
function SearchableItemInput({ value, onChange, onAddNewItem, availableItems, refetch, activeId }) {
  ...
  const handleSelect = (item) => {
    ...
    onChange(activeId, 'price', item.purchasePrice || 0);
  ...
  return (
    <>
      {isShowItemForm && <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 transition-opacity">
        <div className="lg:w-4xl bg-white"> <ItemForm refetch={refetch} setShowForm={setShowItemForm} onClose={() => {
            ...
            {filteredItems.length > 0 ? (
              filteredItems.map((item) => (
                <div
                  ...
                  className="flex text-sm cursor-pointer hover:bg-blue-50 transition-colors"
                >
                  {/* Use item.itemName */}
                  <div className="p-2 w-1/2 text-gray-800 truncate">{item.itemName}</div>
                  {/* Use item.salePrice */}
                  <div className="p-2 w-1/6 text-right text-gray-600">${item.salePrice && item.salePrice.toFixed(2)}</div>
                  {/* Use item.purchasePrice */}
                  <div className="p-2 w-1/6 text-right text-gray-600">${item.purchasePrice && item.purchasePrice.toFixed(2)}</div>
                  {/* Use item.stock.openingQuantity */}
                  <div className={`p-2 w-1/6 text-right font-medium ${item.stock.openingQuantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
...
export default function ItemsTable({ items, onAddItem, onUpdateItem, onDeleteItem, onAddNewItem, itemData, refetch }) {
                ...
                <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <input
                      ...
                      step="0.01"
                      value={item.amount}
                      onChange={(e) => onUpdateItem(item.id, "amount", Number.parseFloat(e.target.value) || 0)}
                      className="w-24 px-2 py-1 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    />
                  </td>
